...
public class Knife4jConfiguration {
	...
	/**
	 * 配置并返回OpenAPI对象，用于描述接口信息 此方法通过Spring的@Bean注解标记，表示该方法的返回值将被Spring容器管理，可被其他组件注入使用
	...
	public OpenAPI apiInfo() {
		...
		return new OpenAPI().components(new Components()
			...
			// 添加参数模板，定义通用的请求头参数
			.addParameters(TOKEN_HEADER, new Parameter().in("header").schema(new StringSchema()).name(TOKEN_HEADER)))
			// 添加API的基本信息，如标题、版本、描述和许可证信息
			.info(new Info().title("Pension Swagger API")
				.version("1.0.0")
